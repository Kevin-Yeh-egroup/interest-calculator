  ...
  const calculateAPR = () => {
    ...
    switch (rateForm.paymentType) {
      case "principal_interest":
        ...
        totalInterest = totalPayment - actualReceived
        calculationSteps.push(
          `3. 本息攤還計算：每期還款 ${paymentAmount} × 總期數 ${totalPeriods} = 總還款 ${totalPayment} 元`,
        )
        calculationSteps.push(`4. 總利息 = 總還款 ${totalPayment} - 實拿金額 ${actualReceived} = ${totalInterest} 元`)
        break
      case "interest_only":
        totalPayment = paymentAmount * totalPeriods
        totalInterest = totalPayment
        calculationSteps.push(
          `3. 只還利息計算：每期還款 ${paymentAmount} × 總期數 ${totalPeriods} = 總利息 ${totalInterest} 元`,
        )
        calculationSteps.push(
          `4. 總還款 = 總利息 ${totalInterest} + 本金 ${actualReceived} = ${totalInterest + actualReceived} 元`,
        )
        break
      case "principal_only":
        totalPayment = paymentAmount * totalPeriods
        ...
        calculationSteps.push(`4. 總利息 = 0 元`)
        break
    }
    ...
    if (rateForm.hasGracePeriod && gracePeriod > 0) {
      ...
      const regularPayment =
        rateForm.paymentType === "principal_interest"
          ...
          : actualReceived + paymentAmount * regularPeriods

      totalPayment = gracePeriodInterest + regularPayment
      calculationSteps.push(`5. 寬限期計算：寬限期 ${gracePeriod} 期，只付利息 ${gracePeriodInterest} 元`)
      calculationSteps.push(`6. 正常還款期 ${regularPeriods} 期，還款金額 ${regularPayment} 元`)
      calculationSteps.push(
        `7. 總還款 = 寬限期利息 + 正常還款 = ${gracePeriodInterest} + ${regularPayment} = ${totalPayment} 元`,
      )
    }

    // 計算總成本（移除隱含費用）
    const totalCost = totalInterest
    calculationSteps.push(`8. 總成本 = 總利息 ${totalInterest} 元`)

    // 計算年化利率
    const averageAnnualCost = totalCost / years
    const apr = (averageAnnualCost / actualReceived) * 100
    calculationSteps.push(`9. 還款年期 = 總期數 ${totalPeriods} ÷ 年頻率 ${periodsPerYear} = ${years.toFixed(2)} 年`)
    calculationSteps.push(
      `10. 平均年成本 = 總成本 ${totalCost} ÷ 還款年期 ${years.toFixed(2)} = ${averageAnnualCost.toFixed(2)} 元/年`,
    )
    calculationSteps.push(
      `11. APR = (平均年成本 ${averageAnnualCost.toFixed(2)} ÷ 實拿金額 ${actualReceived}) × 100% = ${apr.toFixed(2)}%`,
    )
    ...
    if (rateForm.interestType === "simple") {
      // 單利：本利和 = 本金 + (本金 × 利率) × 期間
      simpleInterestRate = (totalInterest / actualReceived / years) * 100
      calculationSteps.push(
        `12. 單利年利率 = (總利息 ${totalInterest} ÷ 實拿金額 ${actualReceived} ÷ 年期 ${years.toFixed(2)}) × 100% = ${simpleInterestRate.toFixed(2)}%`,
      )
    } else {
      // 複利：本利和 = 本金 × (1 + 年利率) ^ 期間
      if (years > 0) {
        compoundInterestRate = (Math.pow((totalPayment + actualReceived) / actualReceived, 1 / years) - 1) * 100
        calculationSteps.push(
          `12. 複利年利率 = (((總還款 ${totalPayment} + 本金 ${actualReceived}) ÷ 本金 ${actualReceived})^(1/${years.toFixed(2)}) - 1) × 100% = ${compoundInterestRate.toFixed(2)}%`,
        )
      }
    }
